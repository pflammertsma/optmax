...
const { instrumentVerdict, fundBreadth } = require('./verdict');
const { costSummary, commissionUsd } = require('./tradecost');
...
function scanInvestments({ holdings = [], cash = 0, targets = [], settings = {}, quotes = {}, watchlistData = [], discovered = [], maxPerCategory = 60 } = {}) {
  ...
  for (const c of candidates) {
    ...
    const lensInput = {
      ...
      analysis, expenseRatioPct: erPct, yieldPct, marketCap, breadth,
    };
    ...
    const price = quote ? (quote.regularMarketPrice ?? null) : null;
    const row = {
      symbol: sym,
      name: c.name,
      bucket: c.bucket,
      kind, isFund, breadth, source: c.source,
      price,
      yieldPct: Math.round(yieldPct * 100) / 100,
      taxDragPct: Math.round(taxDragPct * 100) / 100,
      expenseRatioPct: erPct,
      marketCap,
      isPfic: !!(analysis && analysis.isPfic),
    ...
    // The two things a buyer actually needs: is it worth owning, and what does
    // owning it cost (recurring fee/tax, plus whether the commission is big
    // enough to force a minimum order size / hold period).
    row.verdict = instrumentVerdict(row, { usPerson, dividendTaxRatePct });
    row.cost = costSummary({ price, expenseRatioPct: erPct, yieldPct, dividendTaxRatePct });
  ...
  for (const r of primary) {
    ...
    r.estFeeUsd = commissionUsd(r.suggestedUsd, r.price);
    // Re-cost the row against the actual order size so the UI can say whether
    // this specific trade is big enough for the commission to stop mattering.
    r.cost = costSummary({
      amountUsd: r.suggestedUsd, price: r.price,
      expenseRatioPct: r.expenseRatioPct, yieldPct: r.yieldPct, dividendTaxRatePct,
    });
    cashLeft -= amt;
    filled.add(r.bucket);
  }
