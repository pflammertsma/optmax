...
function parseStatementMeta(text) {
  ...
  // Dividends actually PAID, from the Cash Report section. Gross (Dividends) and
  // the withholding tax deducted, both month- and year-to-date, plus a
  // per-currency breakdown. This is "what hit the account", distinct from the
  // forward projection the app computes from trailing rates.
  // Columns: Cash Report,Data,<row>,<currency>,Total,Securities,Futures,MTD,YTD
  ...
  for (const line of lines) {
    ...
    if (section === 'Cash Report' && cells[1] === 'Data') {
      const row = (cells[2] || '').trim();
      const ccy = (cells[3] || '').trim();
      const mtd = num(cells[7]);
      const ytd = num(cells[8]);
      const isBase = ccy === 'Base Currency Summary';
      if (row === 'Dividends') {
        if (isBase) { div.grossMtd = mtd; div.grossYtd = ytd; }
        else if (/^[A-Z]{3}$/.test(ccy)) { (div.byCurrency[ccy] ??= {}).grossYtd = ytd; }
      } else if (row === 'Withholding Tax') {
        if (isBase) { div.whMtd = mtd; div.whYtd = ytd; }
        else if (/^[A-Z]{3}$/.test(ccy)) { (div.byCurrency[ccy] ??= {}).whYtd = ytd; }
      }
    }
  }

  const hasNav = Object.values(nav).some(v => v != null);
  const hasDiv = div.grossMtd != null || div.grossYtd != null;
  return { statementDate, nav: hasNav ? nav : null, dividendsPaid: hasDiv ? div : null };
}
...
function buildProfileSnapshot(opts) {
  const {
    ...
    pfic = null, nav = null, dividendsPaid = null,
  } = opts;
  ...
  return {
    ...
    // Dividends actually paid (from an imported statement's Cash Report):
    // gross + net-of-withholding, year-to-date. Null on live points (the
    // read-only IBKR sync pulls positions, not the cash ledger).
    dividendsPaidYtd: dividendsPaid ? round2(dividendsPaid.grossYtd) : null,
    dividendsPaidMtd: dividendsPaid ? round2(dividendsPaid.grossMtd) : null,
    dividendsWithheldYtd: dividendsPaid ? round2(dividendsPaid.whYtd) : null,
    dividendsPaidNetYtd: dividendsPaid && dividendsPaid.grossYtd != null
      ? round2(dividendsPaid.grossYtd + (dividendsPaid.whYtd || 0)) : null,
    dividendsPaidByCurrency: dividendsPaid ? dividendsPaid.byCurrency : null,
