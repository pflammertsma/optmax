...
function interpretAuthStatus(status, json) {
  if (status === 401 || status === 403) return { state: 'needs-login', authenticated: false, connected: false };
  if (status !== 200 || !json) return { state: 'unreachable', authenticated: false, connected: false };
  if (json.authenticated) return { state: 'connected', authenticated: true, connected: true, competing: !!json.competing };
  // connected:true here means "session exists, brokerage side not authenticated
  // yet" — getStatus reauthenticates to promote it. connected:false is a true
  // no-session state.
  return { state: 'needs-login', authenticated: false, connected: !!json.connected };
}
