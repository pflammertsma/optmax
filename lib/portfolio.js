...
function parseActivityStatementCsv(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (section === 'Open Positions') {
      if (kind === 'Header') {
        posCols = {};
        cells.forEach((c, idx) => { posCols[c.toLowerCase()] = idx; });
        continue;
      }
      if (kind !== 'Data' || !posCols) continue;
      // Only per-position summaries — skip lot-level and Total rows
      if ((cells[posCols['datadiscriminator']] || '') !== 'Summary') continue;

      const symbol = (cells[posCols['symbol']] || '').toUpperCase().trim();
      if (!symbol || !TICKER_RE.test(symbol)) continue;

      const currency = (cells[posCols['currency']] || '').toUpperCase() || null;
      ...
      const value     = parseNumber(cells[posCols['value']]);
      const costBasis = parseNumber(cells[posCols['cost basis']]);
      const assetCategory = cells[posCols['asset category']] || null;

      holdings.push({
        symbol,
        quantity:      parseNumber(cells[posCols['quantity']]),
