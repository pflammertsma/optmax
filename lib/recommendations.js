...
// Yahoo's dividend fields disagree (and are wrong for some ADRs) — all the
// scale/plausibility handling lives in lib/yield.js so every surface agrees.
...
function buildTaxNotes({ symbol, kind, yieldPct, taxDragPct, dividendTaxRatePct }) {
  ...
  if (yieldPct >= 0.05) {
    notes.push(`~${yieldPct.toFixed(1)}% yield ≈ ${taxDragPct.toFixed(2)}%/yr tax drag at your ${dividendTaxRatePct}% dividend rate (CH taxes dividends as income; US qualified rates apply with <span class="help-tooltip" data-glossary="dividend-tax" style="border-bottom: 1px dotted var(--text-secondary); cursor: pointer;" title="As a US citizen living in Switzerland, you are taxed by both countries on your dividends. Click for glossary.">foreign tax credit interplay</span>).`);
  } else {
...
function generateBuyRecommendations({ holdings = [], cash = 0, targets = [], settings = {}, quotes = {}, watchlistData = [] } = {}) {
  ...
  if (settings.birthYear) {
    ...
    if (actualEquityPct - targetEquityPct > 5) {
      bondsFirst = true;
      glidepathNote = `Equity exposure (${actualEquityPct.toFixed(0)}%) is above your age-${age} <span class="help-tooltip" data-glossary="glidepath" style="border-bottom: 1px dotted rgba(245,158,11,0.5); cursor: pointer;" title="A retirement glidepath target automatically shifts your mix from equities to bonds as you age. Click for glossary.">glidepath target</span> (${targetEquityPct.toFixed(0)}%), so fixed income ranks first.`;
    }
  }
