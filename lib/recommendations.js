...
function buildTaxNotes({ symbol, kind, yieldPct, taxDragPct, dividendTaxRatePct }) {
  ...
  if (yieldPct >= 0.05) {
    notes.push(`~${yieldPct.toFixed(1)}% yield ≈ ${taxDragPct.toFixed(2)}%/yr tax drag at your ${dividendTaxRatePct}% dividend rate (CH taxes dividends as income; US qualified rates apply with foreign tax credit interplay).`);
  } else {
...
function generateBuyRecommendations({ holdings = [], cash = 0, targets = [], settings = {}, quotes = {}, watchlistData = [] } = {}) {
  ...
  if (settings.birthYear) {
    ...
    if (actualEquityPct - targetEquityPct > 5) {
      bondsFirst = true;
      glidepathNote = `Equity exposure (${actualEquityPct.toFixed(0)}%) is above your age-${age} glidepath target (${targetEquityPct.toFixed(0)}%), so fixed income ranks first.`;
    }
  }
