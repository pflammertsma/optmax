...
function computePortfolioHealth({ holdings = [], cash = 0, targets = [], tolerancePct = 5, quotes = {}, settings = {} } = {}) {
  ...
  // The score curve is a fixed calibration; the configurable limit only drives
  // the plain-English action threshold below.
  ...
  breakdown.push({
    ...
    action: concPct > concLimit
      ? `Reduce ${largestStock.symbol} below ${concLimit}% — a systematic sell-down (e.g. a fixed % each quarter) removes the emotion from it.`
