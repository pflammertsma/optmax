...
function generatePortfolioGuidance(holdings = [], cash = 0, targets = [], settings = {}, quotes = {}, watchlistData = []) {
  ...
  // Group PFIC warnings — only relevant for US persons (tax profile in Settings)
  const usPerson = isUSPerson(settings);
  const pficSymbols = [];
  const pficDomiciles = new Set();
  for (const h of usPerson ? holdings : []) {
  ...
  if (pficSymbols.length > 0) {
    ...
    items.push({
      ...
      message: `You hold foreign-domiciled funds: ${pficSymbols.join(', ')} (domiciled in ${domStr}). As a US person, foreign mutual funds or ETFs are classified as <span class="help-tooltip" data-glossary="pfic" style="border-bottom: 1px dotted var(--text-secondary); cursor: pointer;" title="Passive Foreign Investment Company. Click for glossary.">PFICs</span>. They trigger complex IRS <span class="help-tooltip" data-glossary="pfic" style="border-bottom: 1px dotted var(--text-secondary); cursor: pointer;" title="Information Return by a Shareholder of a Passive Foreign Investment Company. Click for glossary.">Form 8621</span> filing requirements and punitive tax rates (up to 37%+ on unrealized gains). Consider replacing them with US-domiciled equivalents.`,
      severity: 'error',
    });
  }
  ...
  // Single-stock/employer ceiling (configurable in Settings). The "critical"
  // threshold is 1.5× the limit (default 10 ⇒ warn >10%, critical >15%).
  const concLimit = settings.concentrationLimitPct ?? 10;
  const concCritical = Math.round(concLimit * 1.5);

  if (employerPct > concCritical) {
    items.push({
      id: 'employer-concentration-hard',
      type: 'concentration',
      title: 'Critical Employer Stock Concentration',
      message: `Your employer stock represents ${employerPct.toFixed(1)}% of your portfolio, exceeding the ${concCritical}% high-risk threshold. Because your salary and career are already linked to this company, we recommend setting up a systematic <span class="help-tooltip" data-glossary="employer-tickers" style="border-bottom: 1px dotted var(--text-secondary); cursor: pointer;" title="A plan to regularly sell employer shares and reinvest in market ETFs. Click for glossary.">diversification plan</span>.`,
      severity: 'error',
    });
  } else if (employerPct > concLimit) {
    items.push({
      id: 'employer-concentration-soft',
      type: 'concentration',
      title: 'Elevated Employer Stock Concentration',
      message: `Your employer stock represents ${employerPct.toFixed(1)}% of your portfolio. We recommend keeping single-stock exposure under ${concLimit}% to reduce <span class="help-tooltip" data-glossary="concentration-limit" style="border-bottom: 1px dotted var(--text-secondary); cursor: pointer;" title="Single-stock risk limit. Click for glossary.">concentration risk</span>.`,
      severity: 'warning',
    });
  }
  ...
  if (cashPct > 10 && cash > cashDragLimit) {
    items.push({
      ...
      message: `You are holding cash representation of ${cashPct.toFixed(1)}% of your portfolio, exceeding your buffer of $${cashDragLimit.toLocaleString()}. Over long horizons, excess cash reduces returns (<span class="help-tooltip" data-glossary="cash-drag" style="border-bottom: 1px dotted var(--text-secondary); cursor: pointer;" title="Cash drag reduces compound growth. Click for glossary.">cash drag</span>). Consider deploying this cash into your Core ETFs.`,
      severity: 'warning',
    });
  }
  ...
  if (settings.birthYear) {
    ...
    if (age > 0) {
      ...
      if (Math.abs(drift) > tolerance) {
        ...
        let actionStr = '';
        if (drift > 0) {
          actionStr = `Action: Sell some equities and buy core bond ETFs (like BND or AGG) to align with your <span class="help-tooltip" data-glossary="glidepath" style="border-bottom: 1px dotted var(--text-secondary); cursor: pointer;" title="Retirement glidepath target. Click for glossary.">retirement target</span>.`;
        } else {
          actionStr = `Action: Buy core equity ETFs (like VTI or VOO) using excess cash or by selling some bonds to align with your <span class="help-tooltip" data-glossary="glidepath" style="border-bottom: 1px dotted var(--text-secondary); cursor: pointer;" title="Retirement glidepath target. Click for glossary.">retirement target</span>.`;
        }
...
function calculateHoldingRecommendation(holding, quote, watchlistInfo, bucketDrifts = [], tolerancePct = 5, totalPortfolioValue = 0, opts = {}) {
  const { usPerson = true } = opts;
  ...
  // (Only for US persons; PFIC is a US-tax concept.)
  if (isFund && usPerson) {
  ...
  const concLimit = opts.concentrationLimitPct ?? 10;

  // 1. Global concentration check: individual stock exceeding the limit is ALWAYS a Trim
  if (!isFund && holdingPct > concLimit) {
    return { type: 'Trim', reason: `Single-stock concentration too high (${holdingPct.toFixed(0)}% vs ${concLimit}% limit)` };
  }
