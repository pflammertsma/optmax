  ...
  if (settings.birthYear) {
    ...
    if (age > 0) {
      ...
      if (Math.abs(drift) > tolerance) {
        ...
        let actionStr = '';
        if (drift > 0) {
          actionStr = `Action: Sell some equities and buy core bond ETFs (like BND or AGG) to align with your retirement target.`;
        } else {
          actionStr = `Action: Buy core equity ETFs (like VTI or VOO) using excess cash or by selling some bonds to align with your retirement target.`;
        }
