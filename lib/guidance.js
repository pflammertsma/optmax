...
function generatePortfolioGuidance(holdings = [], cash = 0, targets = []) {
  const items = [];
  const total = (holdings || []).reduce((s, h) => s + (h.marketValue || 0), 0) + (cash || 0);
  ...
  if (cashPct > 10 && cash > 5000) {
    items.push({
      id: 'cash-drag',
      type: 'cash-drag',
      title: `Cash Drag Alert`,
      message: `You are holding cash representation of ${cashPct.toFixed(1)}% of your portfolio. Over long horizons, excess cash reduces returns. Consider setting up a monthly Dollar-Cost Averaging (DCA) plan to deploy this cash into your Core ETFs.`,
      severity: 'warning',
    });
  }
